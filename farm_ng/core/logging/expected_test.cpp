...
Expected<A> makeA(bool a_error) {
  if (a_error) {
    return FARM_NG_ERROR("a - error");
  }
  return A{.a = "a"};
}

Expected<Ab> makeAb(bool a_error, bool b_error) {
  FARM_NG_TRY(A a, makeA(a_error));

  if (b_error) {
    return FARM_NG_ERROR("b - {}", "error");
  }
  Ab ab;
  ab.a = a;
  ab.b = "bb";
...
Expected<Abc> makeAbc(bool a_error, bool b_error, bool c_error) {
  FARM_NG_TRY(Ab ab, makeAb(a_error, b_error));

  if (c_error) {
    return FARM_NG_ERROR("c - error - {}", 42);
  }
  Abc abc;
  abc.ab = ab;
  abc.c = "ccc";
...
Expected<Abc> makeAbcAtOnce(bool a_error, bool b_error, bool c_error) {
  ...
  if (a_error) {
    error.details.push_back(FARM_NG_ERROR_DETAIL("a - error"));
  } else {
    abc.ab.a.a = "aaa";
  }

  if (b_error) {
    error.details.push_back(FARM_NG_ERROR_DETAIL("b - error"));
  } else {
    abc.ab.b = "BB";
  }
  if (c_error) {
    error.details.push_back(FARM_NG_ERROR_DETAIL("c - error"));
  } else {
    abc.c = "cCc";
  }
...
Expected<A> sum(Expected<A> maybe_left, Expected<A> maybe_right) {
  FARM_NG_TRY(A lhs, maybe_left);
  FARM_NG_TRY(A rhs, maybe_right);
...
TEST(expected, single_error) {
  ...
  FARM_NG_CHECK(!abc);
  FARM_NG_CHECK_EQ(abc.error().details.size(), 1);
...
TEST(expected, multi_error) {
  ...
  FARM_NG_CHECK(!abc);
  FARM_NG_CHECK_EQ(abc.error().details.size(), 2);

  Expected<A> a_good = makeA(false);
  FARM_NG_CHECK(a_good);
...
TEST(expected, optional) {
  ...
  FARM_NG_CHECK(optional_double);
  FARM_NG_CHECK_EQ(FARM_NG_UNWRAP(optional_double), 3);

  expected_double = FARM_NG_ERROR("error");
  ...
  FARM_NG_CHECK(expected_int);
  FARM_NG_CHECK_EQ(FARM_NG_UNWRAP(expected_int), 42);
...
TEST(expected, fancy_error) {
  ...
  Expected<double, MyFancyError> with_fancy_error = FARM_NG_ERROR("failed");
  with_fancy_error.error().diagnostics = 42;
  FARM_NG_CHECK(!with_fancy_error);

  using FancyExpected = Expected<double, MyFancyError>;
  EXPECT_DEATH(
      {
        FancyExpected with_fancy_error = FARM_NG_ERROR("failed");
        with_fancy_error.error().diagnostics = 42;
        FARM_NG_UNWRAP(with_fancy_error);
      },
