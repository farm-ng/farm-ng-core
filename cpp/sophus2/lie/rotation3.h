...
auto rotThroughPoints(
  ...
  if (abs(n) < sophus2::kEpsilon<TScalar>) {
...
namespace details {
template <class TT>
class Cast<sophus2::Rotation3<TT>> {
 public:
  template <class TTo>
  static auto impl(sophus2::Rotation3<TT> const& v) {
    return v.template cast<typename TTo::Scalar>();
  }
  template <class TTo>
  static auto implScalar(sophus2::Rotation3<TT> const& v) {
