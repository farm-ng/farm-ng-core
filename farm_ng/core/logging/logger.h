// Copyright (c) farm-ng, inc. All rights reserved.

#pragma once

#include "farm_ng/core/logging/format.h"

// Begin (Impl details)

#define FARM_NG_IMPL_CHECK_OP(symbol, name_str, lhs, rhs, ...)                 \
  do {                                                                         \
    if (!((lhs)symbol(rhs))) {                                                 \
      FARM_NG_IMPL_LOG_HEADER("CHECK_" name_str " failed");                    \
      FARM_NG_IMPL_LOG_PRINTLN(                                                \
          "Not true: {} " #symbol " {}\n{}\nvs.\n{}\n", #lhs, #rhs, lhs, rhs); \
      FARM_NG_IMPL_LOG_PRINTLN(__VA_ARGS__);                                   \
      FARM_NG_IMPL_ABORT();                                                    \
    }                                                                          \
  } while (false)

#define FARM_NG_IMPL_LOG_HEADER(msg) \
  FARM_NG_IMPL_LOG_PRINTLN("[FARM_NG {} in {}:{}]", msg, __FILE__, __LINE__)
// End (Impl details)

/// Print formatted error message and then panic.
#define FARM_NG_FATAL(cstr, ...)                 \
  FARM_NG_IMPL_LOG_HEADER("FATAL error");        \
  FARM_NG_IMPL_LOG_PRINTLN(cstr, ##__VA_ARGS__); \
  FARM_NG_IMPL_ABORT()

/// Print formatted error message and then panic.
#define FARM_NG_UNIMPLEMENTED(cstr, ...)          \
  FARM_NG_IMPL_LOG_HEADER("NOT IMPLEMENTED YET"); \
  FARM_NG_IMPL_LOG_PRINTLN(cstr, ##__VA_ARGS__);  \
  FARM_NG_IMPL_ABORT()

#define FARM_NG_PRINTLN(cstr, ...) FARM_NG_IMPL_LOG_PRINTLN(cstr, ##__VA_ARGS__)

/// LOG INFO: Log cstr formatted (using variadic arguments) to cerr.
#define FARM_NG_LOG_INFO(cstr, ...)    \
  FARM_NG_IMPL_LOG_HEADER("LOG INFO"); \
  FARM_NG_IMPL_LOG_PRINTLN(cstr, ##__VA_ARGS__)

/// LOG INFO: Only log every nth encounter.
#define FARM_NG_LOG_INFO_EVERY_N(N, cstr, ...)                \
  do {                                                        \
    static std::atomic<int> counter(0);                       \
    ++counter;                                                \
    if (counter > (N)) {                                      \
      counter -= (N);                                         \
    }                                                         \
    if (counter == 1) {                                       \
      FARM_NG_IMPL_LOG_HEADER("LOG INFO EVERY N( =" #N " )"); \
      FARM_NG_IMPL_LOG_PRINTLN(cstr, ##__VA_ARGS__);          \
    }                                                         \
  } while (false)

/// LOG WARNING: Log cstr formatted (using variadic arguments) to cerr.
#define FARM_NG_LOG_WARNING(cstr, ...)    \
  FARM_NG_IMPL_LOG_HEADER("LOG WARNING"); \
  FARM_NG_IMPL_LOG_PRINTLN(cstr, ##__VA_ARGS__)

/// If condition is false, Print formatted error message and then panic.
#define FARM_NG_CHECK(condition, ...)                             \
  do {                                                            \
    if (!(condition)) {                                           \
      FARM_NG_IMPL_LOG_HEADER("FARM_NG_CHECK failed");            \
      FARM_NG_IMPL_LOG_PRINTLN("bool({}) not true.", #condition); \
      FARM_NG_IMPL_LOG_PRINTLN(__VA_ARGS__);                      \
      FARM_NG_IMPL_ABORT();                                       \
    }                                                             \
  } while (false)

/// If it is false that `lhs` == `rhs`, print formatted error message and then
/// panic.
#define FARM_NG_CHECK_EQ(lhs, rhs, ...) \
  FARM_NG_IMPL_CHECK_OP(==, "EQ", lhs, rhs, __VA_ARGS__)

/// If it is false that `lhs` != `rhs`, print formatted error message and then
/// panic.
#define FARM_NG_CHECK_NE(lhs, rhs, ...) \
  FARM_NG_IMPL_CHECK_OP(!=, "NE", lhs, rhs, __VA_ARGS__)

/// If it is false that `lhs` <= `rhs`, print formatted error message and then
/// panic.
#define FARM_NG_CHECK_LE(lhs, rhs, ...) \
  FARM_NG_IMPL_CHECK_OP(<=, "LE", lhs, rhs, __VA_ARGS__)

/// If it is false that `lhs` < `rhs`, print formatted error message and then
/// panic.
#define FARM_NG_CHECK_LT(lhs, rhs, ...) \
  FARM_NG_IMPL_CHECK_OP(<, "LT", lhs, rhs, __VA_ARGS__)

/// If it is false that `lhs` >= `rhs`, print formatted error message and then
/// panic.
#define FARM_NG_CHECK_GE(lhs, rhs, ...) \
  FARM_NG_IMPL_CHECK_OP(>=, "GE", lhs, rhs, __VA_ARGS__)

/// If it is false that `lhs` > `rhs`, print formatted error message and then
/// panic.
#define FARM_NG_CHECK_GT(lhs, rhs, ...) \
  FARM_NG_IMPL_CHECK_OP(>, "GT", lhs, rhs, __VA_ARGS__)

namespace farm_ng_core {
namespace details {
// following:
// https://github.com/strasdat/Sophus/blob/master/sophus/types.hpp#L94
template <class Scalar>
class MaxMetric {
 public:
  static Scalar impl(Scalar s0, Scalar s1) {
    using std::abs;
    return abs(s0 - s1);
  }
};

}  // namespace details

/// Returns maximum metric between two points `p0` and `p1`, with `p0, p1`
/// being matrices or a scalars.
template <class T>
auto maxMetric(T const& p0, T const& p1)
    -> decltype(details::MaxMetric<T>::impl(p0, p1)) {
  return details::MaxMetric<T>::impl(p0, p1);
}
}  // namespace farm_ng_core

/// If it is false that `lhs` is near `rhs` according to threshold `thr`, print
/// formatted error message and then panic.
///
/// `lhs` and `rhs` are near, if maxMetric(lhs, rhs) < thr.
#define FARM_NG_CHECK_NEAR(lhs, rhs, thr, ...)           \
  do {                                                   \
    auto nrm = farm_ng_core::maxMetric((lhs), (rhs));         \
    if (!(nrm < (thr))) {                                \
      FARM_NG_IMPL_LOG_HEADER("CHECK_NEAR failed");      \
      FARM_NG_IMPL_LOG_PRINTLN(                          \
          "Not true: {} near {}; has error of {} (thr: " \
          "{})\n{}\n{}\nvs.\n{}\n{}",                    \
          #lhs,                                          \
          #rhs,                                          \
          nrm,                                           \
          thr,                                           \
          #lhs,                                          \
          lhs,                                           \
          #rhs,                                          \
          rhs);                                          \
      FARM_NG_IMPL_LOG_PRINTLN(__VA_ARGS__);             \
      FARM_NG_IMPL_ABORT();                              \
    }                                                    \
  } while (false)
// End (ASSERT macros)

namespace farm_ng_core {
struct ErrorDetail {
  std::string file;
  int line;
  std::string msg;
};

namespace details {

template <class Container>
auto checkedAtContiguousContainer(
    Container& container,
    size_t index,
    const char* container_cstr,
    const char* index_cstr,
    const std::string& file,
    int line,
    const std::string& str) -> decltype(container[index]) {
  if (index >= container.size()) {
    FARM_NG_IMPL_LOG_PRINTLN("[FARM_NG_AT in {}:{}]", file, line);
    FARM_NG_IMPL_LOG_PRINTLN(
        "index `{}` (={}) not in contiguous container `{}` of size `{}`",
        index_cstr,
        index,
        container_cstr,
        container.size());
    if (!str.empty()) {
      ::fmt::print(stderr, str);
    }
    FARM_NG_IMPL_ABORT();
  }
  return container[index];
}

template <class Container, class Key>
auto checkedGetFromAssociativeContainer(
    Container& container,
    const Key& key,
    const char* container_cstr,
    const char* key_cstr,
    const std::string& file,
    int line,
    const std::string& str) -> decltype(*(container.find(key))) {
  auto it = container.find(key);
  if (it == container.end()) {
    FARM_NG_IMPL_LOG_PRINTLN("[FARM_NG_GET in {}:{}]", file, line);
    FARM_NG_IMPL_LOG_PRINTLN(
        "key `{}` (={}) not in associative container `{}` of size `{}`",
        key_cstr,
        key,
        container_cstr,
        container.size());
    if (!str.empty()) {
      ::fmt::print(stderr, str);
    }
    FARM_NG_IMPL_ABORT();
  }
  return *it;
}

template <class WrapperT>
struct UnwrapImpl {
  static auto impl(
      WrapperT& wrapper,
      const char* wrapper_cstr,
      ::farm_ng_core::ErrorDetail detail) -> decltype(*wrapper) {
    if (!bool(wrapper)) {
      FARM_NG_IMPL_LOG_PRINTLN(
          "[FARM_NG_UNWRAP failed in {}:{}]", detail.file, detail.line);
      FARM_NG_IMPL_LOG_PRINTLN(
          "wrapper type `{}` does not contain a valid value", wrapper_cstr);
      if (!detail.msg.empty()) {
        ::fmt::print(stderr, detail.msg);
      }
      FARM_NG_IMPL_ABORT();
    }
    return *wrapper;
  }
};

template <class WrapperT>
auto unwrapImpl(
    WrapperT& wrapper, const char* wrapper_cstr, ::farm_ng_core::ErrorDetail detail)
    -> decltype(*wrapper) {
  return UnwrapImpl<WrapperT>::impl(wrapper, wrapper_cstr, std::move(detail));
}

}  // namespace details
}  // namespace farm_ng_core

/// Returns `contiguous_container[index]`, but panics if `index` is out of
/// bounds.
#define FARM_NG_AT(contiguous_container, index, ...) \
  farm_ng_core::details::checkedAtContiguousContainer(    \
      contiguous_container,                          \
      index,                                         \
      #contiguous_container,                         \
      #index,                                        \
      __FILE__,                                      \
      __LINE__,                                      \
      FARM_NG_FORMAT(__VA_ARGS__))

/// Returns `associative_container[key]`, but panics if the container does not
/// contain `key` .
#define FARM_NG_GET(associative_container, key, ...)    \
  farm_ng_core::details::checkedGetFromAssociativeContainer( \
      associative_container,                            \
      key,                                              \
      #associative_container,                           \
      #key,                                             \
      __FILE__,                                         \
      __LINE__,                                         \
      FARM_NG_FORMAT(__VA_ARGS__))

/// Returns `*wrapper`, but panics if `wrapper` is `nullopt` or `null`.
#define FARM_NG_UNWRAP(wrapper, ...) \
  farm_ng_core::details::unwrapImpl(      \
      wrapper,                       \
      #wrapper,                      \
      ::farm_ng_core::ErrorDetail{        \
          .file = __FILE__,          \
          .line = __LINE__,          \
          .msg = FARM_NG_FORMAT(__VA_ARGS__)})

/// Checks whether the given path exits.
#define FARM_NG_CHECK_PATH_EXIST(path)                                         \
  do {                                                                         \
    bool does_exist = std::filesystem::exists(path);                           \
    if (!does_exist) {                                                         \
      FARM_NG_IMPL_LOG_HEADER("FARM_NG_CHECK_PATH_EXIST failed");              \
      FARM_NG_IMPL_LOG_PRINTLN("The following path does not exist: {}", path); \
      FARM_NG_IMPL_ABORT();                                                    \
    }                                                                          \
  } while (false)

/// Checks whether the given path is a regular file.
/// Hence it checks whether
///  a) it does exit,
///  b) it is a regular file.
#define FARM_NG_CHECK_IS_FILE(path)                              \
  do {                                                           \
    FARM_NG_CHECK_PATH_EXIST(path);                              \
    bool is_regular = std::filesystem::is_regular_file(path);    \
    if (!is_regular) {                                           \
      FARM_NG_IMPL_LOG_HEADER("FARM_NG_CHECK_IS_FILE failed");   \
      FARM_NG_IMPL_LOG_PRINTLN(                                  \
          "The following path is not a regular file: {}", path); \
      FARM_NG_IMPL_ABORT();                                      \
    }                                                            \
  } while (false)

/// Checks whether the given path is a folder.
/// Hence it checks whether
///  a) it does exit,
///  b) it is a folder (and not a file).
#define FARM_NG_CHECK_IS_FOLDER(path)                            \
  do {                                                           \
    FARM_NG_CHECK_PATH_EXIST(path);                              \
    bool is_folder = std::filesystem::is_directory(path);        \
    if (!is_folder) {                                            \
      FARM_NG_IMPL_LOG_HEADER("FARM_NG_CHECK_IS_FOLDER failed"); \
      FARM_NG_IMPL_LOG_PRINTLN(                                  \
          "The following path is not a folder: {}", path);       \
      FARM_NG_IMPL_ABORT();                                      \
    }                                                            \
  } while (false)

/// Checks whether the given path is an empty folder.
/// Hence it checks whether
///  a) it does exit,
///  b) it is a folder (and not a file),
///  c) it is empty
#define FARM_NG_CHECK_FOLDER_IS_EMPTY(path)                                    \
  do {                                                                         \
    FARM_NG_CHECK_IS_FOLDER(path);                                             \
    bool is_empty = std::filesystem::is_empty(path);                           \
    if (!is_empty) {                                                           \
      FARM_NG_IMPL_LOG_HEADER("FARM_NG_CHECK_FOLDER_IS_EMPTY failed");         \
      FARM_NG_IMPL_LOG_PRINTLN("The following folder is not empty: {}", path); \
      FARM_NG_IMPL_ABORT();                                                    \
    }                                                                          \
  } while (false)
