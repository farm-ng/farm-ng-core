...
class Ray {
 public:
  ...
  [[nodiscard]] auto intersect(Eigen::Hyperplane<TScalar, kN> const& plane)
    ...
    if (abs(dot_prod) < sophus2::kEpsilon<TScalar>) {
      return std::nullopt;
    }
...
auto closestApproachParameters(Ray3<TT> const& line_0, Ray3<TT> const& line_1)
  ...
  // ```
  //    x(lambda0)-y(lambda1) = theta*(d0 X d1), for free scalar theta.
  // => o0-o1 + lambda0*d0 - lambda1*d1 - theta*(d0Xd1) = 0
  // => (d0|-d1|-d0Xd1).(lambda0,lambda1,theta)^T = -(o0-o1)
  //           A       .        x               =   b
  // ```
  ...
  if (d0_cross_s1_length < kEpsilon<TT>) {
    // Rays are parallel so no unique solution exists.
    return std::nullopt;
  }
