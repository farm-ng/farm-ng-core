...
#include "sophus2/image/layout.h"
...
struct ImageView {
  ...
  explicit ImageView(sophus2::ImageSize image_size, TPixel const* ptr) noexcept
  ...
  [[nodiscard]] auto imageSize() const -> sophus2::ImageSize const& {
  ...
  /// Returns subview.
  [[nodiscard]] auto subview(Eigen::Vector2i uv, sophus2::ImageSize size) const
      -> ImageView {
...
template <class TPixel>
auto finiteInterval(sophus2::ImageView<TPixel> const& image) -> Region<TPixel> {
  return image.reduce(
...
inline auto imageCoordsInterval(
    sophus2::ImageView<TPixel> const& image, int border = 0) -> Region2I {
...
#define SOPHUS_PIXEL_MUT(img, u, v, ...)     \
  ::sophus2::details::checkedPixelAccessMut( \
