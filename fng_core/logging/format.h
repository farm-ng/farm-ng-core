...
#define FNG_IMPL_LOG_PRINTLN_ONE(cstr) fng_core::logLine(FNG_STRING(cstr))

#define FNG_IMPL_LOG_PRINTLN_ARGS(cstr, ...) \
  fng_core::logLine(::fmt::format(FMT_STRING(cstr), __VA_ARGS__))
...
#define FNG_RUNTIME_FORMAT(str, ...) \
  ::fng_core::details::runtimeFormatImpl(__FILE__, __LINE__, str, __VA_ARGS__)
