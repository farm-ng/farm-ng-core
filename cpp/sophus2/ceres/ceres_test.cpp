...
#include "sophus2/lie/spiral_similarity2.h"
#include "sophus2/lie/spiral_similarity3.h"
...
struct SimplePriorProblem {
  ...
  void solve(Variables& estimate) {
    ...
    auto parametrization = new sophus2::ceres::Manifold<TTransform>;
...
struct SensorMeasurementProblem {
  ...
  struct CostFunctor {
    CostFunctor(
        sophus2::PinholeModel const& camera_model,
    ...
    sophus2::PinholeModel camera_model;
  ...
  SensorMeasurementProblem(
    ...
    int width = 640;
    int height = 480;
    sophus2::PinholeModel pinhole_intrinsics =
        createDefaultPinholeModel({width, height});

    Sensor cam_right;
    cam_right.intrinsics = pinhole_intrinsics;
    cam_right.robot_from_camera = sophus2::Isometry3F64::fromTy(0.25);

    Sensor cam_left;
    cam_left.intrinsics = pinhole_intrinsics;
    cam_left.robot_from_camera = sophus2::Isometry3F64::fromTy(-0.25);
...
TEST(ceres_solve, regression_test) {
  ...
  static double const kScaleThr = 0.005;
  {
    auto perturb = [](Isometry2F64& est) {
      ...
      est.setRotation(est.rotation() * sophus2::Rotation2F64(0.1));
    };
...

  {
    auto perturb = [](Isometry3F64& est) {
      ...
      est.setRotation(est.rotation() * sophus2::Rotation3F64::fromRx(0.1));
    };
    ...
    SensorMeasurementProblem<Isometry3> sensor_problem(
        [](int kNumPoses) -> std::vector<Isometry3F64> {
          ...
          for (int i = 0; i < kNumPoses; ++i) {
            path.push_back(sophus2::Isometry3F64::fromTx(0.1 * i));
          }
          return path;
        });
...
TEST(jet_lie_group, compile_test) {
  JetLieGroupTests<sophus2::Identity2>::testAll();
  JetLieGroupTests<sophus2::Identity3>::testAll();
  JetLieGroupTests<sophus2::Translation2>::testAll();
  JetLieGroupTests<sophus2::Translation3>::testAll();

  JetLieGroupTests<sophus2::Rotation2>::testAll();
  JetLieGroupTests<sophus2::Rotation3>::testAll();
  JetLieGroupTests<sophus2::Isometry2>::testAll();
  JetLieGroupTests<sophus2::Isometry3>::testAll();

  JetLieGroupTests<sophus2::Scaling2>::testAll();
  JetLieGroupTests<sophus2::Scaling3>::testAll();
  JetLieGroupTests<sophus2::ScalingTranslation2>::testAll();
  JetLieGroupTests<sophus2::ScalingTranslation3>::testAll();

  JetLieGroupTests<sophus2::SpiralSimilarity2>::testAll();
  JetLieGroupTests<sophus2::SpiralSimilarity3>::testAll();
  JetLieGroupTests<sophus2::Similarity2>::testAll();
  JetLieGroupTests<sophus2::Similarity3>::testAll();
}
